...
function App() {
  const [engStreetName, setEngStreetName] = useState('');
  const [chiStreetName, setChiStreetName] = useState('');
  const [foundStreets, setFoundStreets] = useState(STREETS);
  ...
  const filter = (e) => {
    ...
    if (keyword !== '') {
      const results = STREETS.filter((street) => {
        return street.engStreetName.toLowerCase().startsWith(keyword.toLowerCase());
        // Use the toLowerCase() method to make it case-insensitive
      });
      setFoundStreets(results);
    } else {
      setFoundStreets(STREETS);
      // If the text field is empty, show all users
    }

    setEngStreetName(keyword);
  };
